...
def get_quote_from_brl_to_eur():
    """Get quote from BRL to EUR.

    This is the kind of code that will live in another
    module, not in the CLI. It is here now for the
    example purpose."""
    typer.echo("Getting quote from BRL to EUR...")
...
def parse_the_data():
    ...
    sub_steps = [
        Step(
            ...
            sub_steps=[],
            function=get_quote_from_brl_to_eur,
        ),
        Step(
...
def generate_the_report():
    typer.echo("""Well done!""")


if __name__ == "__main__":
    typer.secho("My expenses", fg=typer.colors.BRIGHT_BLACK)
    typer.secho(
        """This is a step by step on how to retrieve data
        from my banks and generate a report with it.""",
